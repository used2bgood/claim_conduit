...
import { CheckSquare, Plus, Edit, Trash2, RotateCcw, AlertTriangle } from "lucide-react";
...
export default function TaskStatusManager() {
    ...
    const loadStatuses = async () => {
        setIsLoading(true);
        try {
            const [taskStatuses, allTasks] = await Promise.all([
            ...
            taskStatuses.forEach(status => {
                usage[status.label] = allTasks.filter(task => task.status === status.label).length;
            });
    ...
    const handleDelete = async (status) => {
        ...
        if (usageCount > 0) {
            alert(`Cannot delete "${status.label}" - it's being used by ${usageCount} task(s). Please reassign those tasks first.`);
            return;
        }
    ...
    const handleBulkRename = async () => {
        ...
        setIsUpdating(true);
        try {
            // First, update all tasks that use this status
            const tasksToUpdate = await Task.filter({ status: renameData.oldStatus.label });
            
            const updatePromises = tasksToUpdate.map(task => 
                Task.update(task.id, { status: renameData.newLabel })
            );
        ...
        <Card className="bg-white shadow-sm border-0">
            ...
            <CardContent>
                <p className="text-sm text-gray-600 mb-4">
                    Manage the status labels for tasks. Usage counts show how many tasks currently use each status.
                </p>
                <div className="mb-4 p-3 bg-green-50 rounded-lg border border-green-200">
                    <p className="text-sm font-medium text-green-800 mb-2">These statuses are used in:</p>
                    <ul className="text-sm text-green-700 space-y-1">
                        <li>• Task Management page (task status badges)</li>
                        <li>• Create Task form (status dropdown)</li>
                        <li>• Task Details page (status dropdown and display)</li>
                        <li>• Client Profile pages (task status badges)</li>
                    </ul>
                </div>
                <div className="space-y-2">
                    {isLoading ? (
                        ...
                        <div className="text-center py-8">
                            <p className="text-gray-500 mb-4">No task statuses found. Create your first one!</p>
                        ...
                        <div key={status.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 border border-gray-100">
                            <div className="flex items-center gap-3">
                                <Badge className={`${status.color_bg} ${status.color_text} text-base py-1 px-3`}>
                                    {statusDisplayNames[status.label] || status.label}
                                </Badge>
                                <span className="text-sm text-gray-500">
                                    {statusUsage[status.label] || 0} task(s) using this status
                                </span>
                            </div>
                            <div className="flex items-center gap-2">
            ...
            <Dialog open={showDialog} onOpenChange={setShowDialog}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{editingStatus?.id ? 'Edit' : 'Add'} Task Status</DialogTitle>
                    </DialogHeader>
            ...
            <Dialog open={showRenameDialog} onOpenChange={setShowRenameDialog}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Rename Task Status</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div className="flex items-center gap-2 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                            <AlertTriangle className="w-5 h-5 text-yellow-600" />
                            <div className="text-sm text-yellow-800">
                                This will rename the status and update all {statusUsage[renameData.oldStatus?.label] || 0} tasks that currently use this status.
                            </div>
                        </div>
